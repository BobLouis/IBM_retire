...
class NN_process():
    ...
    def __init__(self):
        ...
        self.ReplaceName_Pred = {
                        ...
                        '職級代碼': 'level',
                        # leave
                        '今天之後有請過假': 'leave_after_today','兩個月平均事假次數': 'personal_leave_60_count', '三個月平均事假次數': 'personal_leave_90_count', '六個月平均事假次數': 'personal_leave_180_count',
                        '兩個月平均年假次數': 'year_leave_60_count', '三個月平均年假次數': 'year_leave_90_count', '六個月平均年假次數': 'year_leave_180_count',
                        '兩個月平均非住院病假次數': 'sick_leave_60_count', '三個月平均非住院病假次數': 'sick_leave_90_count', '六個月平均非住院病假次數': 'sick_leave_180_count',
                        '六個月平均非住院病假+事假次數': 'sick_leave_personal_leave_180_count',
                        '三個月平均非住院病假+事假次數': 'sick_leave_personal_leave_90_count',
    ...
    def train(self, Today):
        ...
        for RATIO in importance_ratio:
            ...
            del_list = self.CutList(importance_result, RATIO)
            for d in del_list:
            ...
            if TRAIN_AND_TEST:
                ...
                MLP = MLPClassifier(solver = 'sgd', hidden_layer_sizes = (150,100,50), learning_rate = 'adaptive',
                                    ...
                                    activation = 'relu')
                MLP.fit(X, Y)
                dump(MLP, './models/MLP_' + GlobalName + '.joblib')
                # 計算AUC threshold
                # 重新預測訓練資料以計算AUC threshold
                my_predict_proba = MLP.predict_proba(X)
                fpr, tpr, threshold = metrics.roc_curve(Y, my_predict_proba[:,1], pos_label=1)
    ...
    def predict(self, Today, scaler):
        ...
        # load model     
        if MODEL == 2:
            MLP = load('./models/MLP_' + GlobalName + '.joblib') 

            AUC = True
            # 原先以AUC threshold形式決定預測結果(1 or 0)，最後改為輸出離職機率(1~0)
            if AUC:
                data_selected = scaler.transform(data_selected)
                predict = np.array(MLP.predict_proba(data_selected))
                for i in predict:
                    print(i)
                ...
                predict = MLP.predict(data_selected)
                print(predict)

            predict_prob = np.array(MLP.predict_proba(data_selected))[:, 1]
            ...
            if len(predict) == len(code):               
                #with open('./probability/NN_'+GlobalName+'.csv', 'w', newline='') as csvfile:
                ##########
                # 模式二 #
                ##########
                with open('./probability/NN_'+str(datetime.datetime.now())[:10]+'.csv', 'w', newline='') as csvfile:
                ##########
                # 模式二 #
                ##########
                    ...
                    for i in range(len(predict)):
                        writer.writerow([code[i],  str(datetime.datetime.now())[:19],predict_prob[i], 'DEPT', 'NN'])
            else:
                print('length of data and prediction doesn\'t match!')
                print(len(predict))
                print(len(code))
        save_path = './probability/NN_'+GlobalName+'.csv'
        ##########
        # 模式二 #
        ##########
        save_path = './probability/NN_'+str(datetime.datetime.now())[:10]+'.csv'
        ##########
        # 模式二 #
        ##########
